...
// Locked sessions (per activity) stored in plugin config.
$lockkey   = 'locks_' . $cm->id;
$lockedraw = (string)get_config('mod_workshopbooking', $lockkey);
$lockedids = json_decode($lockedraw !== '' ? $lockedraw : '[]', true);
if (!is_array($lockedids)) { $lockedids = []; }
...
if (!function_exists('wsb_parse_workshopnames')) {
    function wsb_parse_workshopnames(string $raw): array {
        ...
        foreach (preg_split('/\r?\n/', trim((string)$raw)) as $line) {
            ...
            $key = wsb_display_name((string)($parts[0] ?? ''));
            if ($key === '') { continue; }
            $url  = $parts[2] ?? '';
            $desc = '';
            $mode = 'beides';
            ...
            // Optional room column
            if (isset($parts[5])) {
                // With explicit mode: part[4] is room, part[5] is desc
                $room = trim((string)($parts[4] ?? ''));
                $desc = $parts[5] ?? $desc;
            } else if (!in_array($p3, ['vm','nm','beides']) && isset($parts[4])) {
                // No mode but 5 parts: part[3] is room, part[4] is desc
                $room = trim((string)($parts[3] ?? ''));
                $desc = $parts[4] ?? $desc;
            }
            // Support line breaks written as '|' or as literal \n sequences
            $desc = str_replace(["\\r\\n","\\n"], "\n", $desc);
            $desc = preg_replace('/\s*\|\s*/', "\n", $desc);
            $map[$key] = ['url' => $url, 'desc' => $desc, 'mode' => $mode, 'room' => $room];
...
if (!function_exists('workshopbooking_ensure_group_for_session')) {
    ...
    function workshopbooking_ensure_group_for_session($session, $cm, $instance, $createmissing = true) {
        ...
        if (!has_capability('moodle/course:managegroups', $contextcourse)) { return 0; }
        try {
...
if ($admact && $bid && $canmanage) {
    ...
    if ($admact === 'confirm') {
        $gid = workshopbooking_ensure_group_for_session($session, $cm, $instance, false);
        if ($gid) {
            require_once($CFG->dirroot.'/group/lib.php');
...
/* ===== Lock / Unlock a single session (manager only) ===== */
if ($action === 'locksession' && $canmanage) {
    require_sesskey();
    $sid = required_param('sessionid', PARAM_INT);
    $new = array_values(array_unique(array_merge($lockedids, [(int)$sid])));
    set_config($lockkey, json_encode($new), 'mod_workshopbooking');
    redirect(new moodle_url('/mod/workshopbooking/view.php', ['id'=>$cm->id]), get_string('lock_set','mod_workshopbooking'), 0);
}
if ($action === 'unlocksession' && $canmanage) {
    require_sesskey();
    $sid = required_param('sessionid', PARAM_INT);
    $new = array_values(array_diff($lockedids, [(int)$sid]));
    set_config($lockkey, json_encode($new), 'mod_workshopbooking');
    redirect(new moodle_url('/mod/workshopbooking/view.php', ['id'=>$cm->id, 'showhidden'=>1]), get_string('lock_unset','mod_workshopbooking'), 0);
}
...
if ($action === 'details' && $sessionid) {
    ...
    $islocked = in_array((int)$sessionid, $lockedids, true);
    $hasbooking = $DB->record_exists('workshopbooking_booking', ['sessionid'=>$sessionid,'userid'=>$USER->id,'status'=>1]);
    if ($islocked && !$canmanage && !$hasbooking) {
        redirect(new moodle_url('/mod/workshopbooking/view.php', ['id'=>$cm->id]), get_string('locked_text','mod_workshopbooking'), 0);
    }


    // Format strings
    $formatday = get_string('strftimedaydate', 'core_langconfig');
$formattime = get_string('strftimetime', 'core_langconfig');
$startstr = userdate((int)$s->timestart, $formatday);
...
$hourunit = $durationhours === 1 ? get_string('hour', 'core') : get_string('hours', 'core');
$durationtext = get_string('durationlabel','mod_workshopbooking').' '.$durationhours.' '.$hourunit;
$counts  = \mod_workshopbooking\local\bookingmanager::session_counts($s->id);
...
// === Meine Buchungen Button als Header-Action (versteckt bei Details/Ajax) ===
if (isset($PAGE, $cm)) {
    $wb_action = optional_param('action', '', PARAM_ALPHA);
    $wb_ajax   = optional_param('ajax', 0, PARAM_BOOL);
    $label = get_string('mybookings', 'mod_workshopbooking');
    if ($label === '[[mybookings]]') { $label = 'Meine Buchungen'; }
    if (!$wb_ajax && $wb_action !== 'details') {
        $PAGE->set_button(
            html_writer::link(
                new moodle_url('/mod/workshopbooking/mybookings.php', ['id'=>$cm->id]),
                $label,
...
echo $OUTPUT->header();
echo html_writer::tag('style', '.sb-card{position:relative}.wsb-fab{position:absolute;bottom:8px;right:8px;text-decoration:none}.wsb-fab img{width:24px;height:24px;display:block;object-fit:contain}', ['type'=>'text/css']);
...
// Show blackout dates box under filters (trainers/managers only)
if (!empty($istrainer_any)) {
$__blocked = trim((string)get_config('mod_workshopbooking','blockeddates'));
if ($__blocked !== '') {
    echo html_writer::div(
        html_writer::tag('strong', get_string('blockeddates','mod_workshopbooking')) .
        html_writer::tag('pre', s($__blocked), ['class'=>'wb-blocked-list']),
        'wb-blocked-box'
    );
}
}
...
// --- Fallback: Button im Inhalt, falls Theme set_button nicht rendert ---
if (isset($cm, $OUTPUT)) {
    static $wbMyBtn = false;
    if (!$wbMyBtn) {
        $wbMyBtn = true;
        $label = get_string('mybookings', 'mod_workshopbooking');
        if ($label === '[[mybookings]]') { $label = 'Meine Buchungen'; }
        echo html_writer::div(
            html_writer::link(new moodle_url('/mod/workshopbooking/mybookings.php', ['id'=>$cm->id]), $label, ['class'=>'btn btn-secondary']),
            'mb-3'
        );
    }
}
    ...
    $mwmap = wsb_parse_workshopnames((string)($instance->workshopnames ?? ''));
    $mwinfo = $mwmap[$displayname] ?? ($mwmap[core_text::strtoupper($displayname)] ?? ($mwmap[core_text::strtolower($displayname)] ?? ['url'=>'', 'desc'=>'', 'room'=>'']));
    ...
    if (!empty($mwinfo['room'])) { echo html_writer::tag('div', html_writer::tag('span', 'Raum ' . format_string($mwinfo['room']), ['style'=>'font-weight:600;color:#14458f']), ['class'=>'mb-1']); }
    echo html_writer::tag('div', get_string('startlabel','mod_workshopbooking').' '.$startstr, ['class'=>'mb-1']);
    echo html_writer::tag('div', $durationtext, ['class'=>'mb-2']);
...
$title    = format_string($displayname !== '' ? $displayname : wsb_str('pluginname','Workshop'));
    $mwmap = wsb_parse_workshopnames((string)($instance->workshopnames ?? ''));
    $mwinfo = $mwmap[$displayname] ?? ($mwmap[core_text::strtoupper($displayname)] ?? ($mwmap[core_text::strtolower($displayname)] ?? ['url'=>'', 'desc'=>'', 'room'=>'']));
    $startstr = userdate((int)$s->timestart, get_string('strftimedaydate','core_langconfig'));
    $durationhours = isset($instance->durationhours) ? (int)$instance->durationhours : (int)$instance->durationdays;
    $durationhours = max(1, $durationhours);
    ...
    if ($already && (int)$already->status === 1) {
        ...
        if (!$s->_open) {
            ...
            if ($s->_isfull) {
                ...
                if (!$istrainer_any) {
                    // Sperrtage blockieren: keine Anmeldung anzeigen
                    if (function_exists('wsb_blocked_label')) {
                        $__bl = wsb_blocked_label((int)$s->timestart);
                    }
                    if (!empty($__bl)) {
                        $__iso = userdate((int)$s->timestart, '%Y-%m-%d');
                        $chips[] = sb_status_chip('closed', s($__bl));
                    } else {
    ...
    $islocked = in_array((int)$s->id, $lockedids, true);
    $hasbooking = $DB->record_exists('workshopbooking_booking', ['sessionid'=>$s->id,'userid'=>$USER->id,'status'=>1]);
    if ($islocked && !$canmanage && !$hasbooking) { continue; }
    echo html_writer::start_div('sb-card', ['style'=>'position:relative']);
        echo html_writer::start_div('sb-card__head');
            if ($islocked) { echo html_writer::div(get_string('locked_badge','mod_workshopbooking'), 'sb-badge', ['style'=>'background:#dc3545;color:#fff;position:absolute;top:4px;right:4px;z-index:10']); }
            echo html_writer::tag('h3', $title, ['class' => 'sb-title']);
        echo html_writer::end_div();
        if ($slot) { echo sb_slot_badge($slot); }
        if (!empty($mwinfo['room'])) { echo html_writer::div(html_writer::tag('span', 'Raum ' . format_string($mwinfo['room']), ['style'=>'font-weight:600;color:#14458f']), 'sb-room'); }
        echo html_writer::div(get_string('startlabel','mod_workshopbooking').' '.$startstr, 'sb-sub');
        echo html_writer::div($durationtext, 'sb-sub');
...
if ($chips) { echo html_writer::div(implode(' ', $chips), 'sb-statuses'); }
    ...
    if ($islocked) { $btn = html_writer::div(get_string('locked_text','mod_workshopbooking'), 'text-muted'); }
    echo html_writer::div($btn, 'sb-actions');

    // Manager: lock/unlock toggle
    if ($canmanage) {
        $formaction = (new moodle_url('/mod/workshopbooking/view.php'))->out(false);
        if ($islocked) {
            $unlockurl = new moodle_url('/mod/workshopbooking/view.php', ['id'=>$cm->id,'action'=>'unlocksession','sessionid'=>$s->id,'sesskey'=>sesskey()]);
            echo html_writer::link($unlockurl, html_writer::empty_tag('img', ['src'=>$OUTPUT->image_url('unlocksession','mod_workshopbooking'),'alt'=>get_string('btn_unlock','mod_workshopbooking'),'style'=>'width:24px;height:24px;display:block;object-fit:contain']), ['class'=>'wsb-fab wsb-fab-unlock', 'style'=>'position:absolute;bottom:8px;right:8px', 'title'=>get_string('btn_unlock','mod_workshopbooking'), 'aria-label'=>get_string('btn_unlock','mod_workshopbooking')]);
        } else {
            $lockurl = new moodle_url('/mod/workshopbooking/view.php', ['id'=>$cm->id,'action'=>'locksession','sessionid'=>$s->id,'sesskey'=>sesskey()]);
            echo html_writer::link($lockurl, html_writer::empty_tag('img', ['src'=>$OUTPUT->image_url('locksession','mod_workshopbooking'),'alt'=>get_string('btn_lock','mod_workshopbooking'),'style'=>'width:24px;height:24px;display:block;object-fit:contain']), ['class'=>'wsb-fab wsb-fab-lock', 'style'=>'position:absolute;bottom:8px;right:8px', 'title'=>get_string('btn_lock','mod_workshopbooking'), 'aria-label'=>get_string('btn_lock','mod_workshopbooking'), 'onclick'=>"return confirm('".get_string('confirm_lock','mod_workshopbooking')."')"]);
        }
    }
